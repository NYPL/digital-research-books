        ...
        with patch('processes.grin.conversion.DBManager') as MockDBManager:
            with patch('managers.SQSManager') as MockSQSManager:
                with patch('processes.grin.conversion.utils.parse_process_args') as mock_parse_args:
                    ...
                    MockDBManager.return_value.__enter__.return_value = mock_db_manager_instance
...
def test_convert_barcodes_pending_conversion_fix(mocked_conversion_process):
    ...
    with patch.object(conversion_process_instance, "_convert_barcodes", return_value=(mock_converting, mock_converted, mock_unavailable)) as mock_convert_barcodes:
        with patch.object(conversion_process_instance, "_update_grin_state") as mock_update_grin_state:
            ...
            mock_update_grin_state.assert_any_call(mock_converting, old_state=GRINState.PENDING_CONVERSION, new_state=GRINState.CONVERTING)
            mock_update_grin_state.assert_any_call(mock_converted, old_state=GRINState.PENDING_CONVERSION, new_state=GRINState.CONVERTED)
            mock_update_grin_state.assert_any_call(mock_unavailable, old_state=GRINState.PENDING_CONVERSION, new_state=GRINState.UNAVAILABLE)