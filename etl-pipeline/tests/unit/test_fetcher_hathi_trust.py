...
class TestHathiFetcher:
    ...
    def mock_mets_object(self):
        return {"METS:structMap": {"METS:div": {"METS:div": [i for i in range(25)]}}}
    ...
    def test_fetch_volume_cover_success(
        self, test_fetcher, mock_mets_object, mock_pages, mocker
    ):
        ...
        mock_set_cover_page_url = mocker.patch.object(
            HathiFetcher, "set_cover_page_url"
        )
        ...
        mock_request.assert_called_once_with(
            "test_api_root/structure/1?format=json&v=2"
        )

    def test_fetch_volume_cover_mets_error(
        self, test_fetcher, mock_mets_object, mocker
    ):
        mock_response = mocker.MagicMock()
        mock_mets_object["METS:structMap"]["METS:div"]["METS:div"] = {
            i: "data" for i in range(25)
        }
        mock_response.json.return_value = mock_mets_object
        mock_request = mocker.patch.object(HathiFetcher, "make_hathi_req")
        mock_request.return_value = mock_response
    ...
    def test_generate_auth(self, test_fetcher, mocker):
        ...
        mock_auth.assert_called_once_with(
            "test_api_key", client_secret="test_api_secret", signature_type="query"
        )
