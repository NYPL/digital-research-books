...
class TestOpenLibraryFetcher:
    ...
    def test_has_cover_true(self, test_fetcher, mocker):
        mock_volume_cover = mocker.patch.object(
            OpenLibraryFetcher, "fetch_volume_cover"
        )
    ...
    def test_fetch_volume_cover_success(self, test_fetcher, mocker):
        mock_set_cover_page_url = mocker.patch.object(
            OpenLibraryFetcher, "set_cover_page_url"
        )
        ...
        assert (
            test_fetcher.session.query()
            .filter.call_args[0][0]
            .compare((OpenLibraryCover.name == "test"))
        )
        assert (
            test_fetcher.session.query()
            .filter()
            .filter.call_args[0][0]
            .compare((OpenLibraryCover.value == 1))
        )
