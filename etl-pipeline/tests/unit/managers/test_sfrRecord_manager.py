...
class TestSFRRecordManager:
    ...
    def test_mergeRecords(self, testInstance, mocker):
        ...
        testInstance.work.editions = [
            mocker.MagicMock(
                identifiers=["id1", "id2"],
                items=firstEdItems,
                dcdw_uuids=["uuid1", "uuid2"],
            ),
            mocker.MagicMock(
                identifiers=["id3"], items=secondEdItems, dcdw_uuids=["uuid4"]
            ),
        ]
    ...
    def test_dedupeIdentifiers(self, testInstance, mocker):
        ...
        assert testIdentifiers == {("test", 3): 5, ("test", 1): 6}
    ...
    def test_buildEditionStructure(self, testInstance, mocker):
        ...
        testEditions, testWorkInstances = testInstance.buildEditionStructure(
            mockRecords,
            [(1900, ["uuid1", "uuid2", "uuid3"]), (2000, ["uuid4", "uuid5"])],
        )
    ...
    def test_buildWork(self, testInstance, mocker):
        ...
        managerMocks["buildEditionStructure"].assert_called_once_with(
            "testRecords", "testEditions"
        )
        managerMocks["createEmptyWorkRecord"].assert_called_once()
        managerMocks["addWorkInstanceMetadata"].assert_has_calls(
            [
                mocker.call("testWorkData", "workInst1"),
                mocker.call("testWorkData", "workInst2"),
            ]
        )
        managerMocks["buildEdition"].assert_has_calls(
            [
                ...
                mocker.call(
                    "testWorkData", 2000, ["instance3", "instance4", "instance5"]
                ),
            ]
        )
    ...
    def test_buildEdition(self, testInstance, mocker):
        mockEmptyEdition = mocker.patch.object(
            SFRRecordManager, "createEmptyEditionRecord"
        )
        ...
        assert testWorkData["editions"] == [
            {"title": "Test Edition", "publication_date": 1900}
        ]
    ...
    def test_parseInstance(self, testInstance, testDCDWRecord, mocker):
        ...
        mockItemBuild.assert_called_once_with(
            testEdition, testDCDWRecord, set(["Contrib 2|||provider"])
        )
    ...
    def test_buildItems(self, testInstance, testDCDWRecord):
        ...
        testInstance.buildItems(
            testEditionData, testDCDWRecord, set(["Item Contrib 1"])
        )
        ...
        assert (
            testEditionData["items"][0]["publisher_project_source"]
            == "TestPubProjSource"
        )
        ...
        assert testEditionData["items"][0]["physical_location"] == {
            "code": "tst",
            "name": "Test Location",
        }
    ...
    def test_publicationDateCheck5(self):
        ...
        testEdition5["publication_date"] = datetime.now(timezone.utc).replace(
            tzinfo=None
        ) + timedelta(1)
        testPubDateCheck5 = SFRRecordManager.publicationDateCheck(testEdition5)
        assert testPubDateCheck5 == None

        testEdition5["publication_date"] = (
            datetime.now(timezone.utc).replace(tzinfo=None) + timedelta(1)
        ).strftime("%Y-%m-%d")
        testPubDateCheck5 = SFRRecordManager.publicationDateCheck(testEdition5)
        assert testPubDateCheck5 == None
    ...
    def test_setPipeDelimitedData(self, mocker):
        ...
        testParsedData = SFRRecordManager.setPipeDelimitedData(
            [1, 2, 3, 4, 5], "testFields", dType="testType", dParser="testFunction"
        )
    ...
    def test_parseDelimitedEntry_custom_function_no_type(self, mocker):
        ...
        parsedData = SFRRecordManager.parseDelimitedEntry(
            "1|2|3", ["f1", "f2", "f3"], None, dParser=mockParser
        )
    ...
    def test_parseDelimitedEntry_custom_function_custom_type(self, mocker):
        ...
        mockParser.return_value = {"tests": [1, 2, 3]}
        parsedData = SFRRecordManager.parseDelimitedEntry(
            "1|2|3", ["f1", "f2", "f3"], dType=mocker.MagicMock, dParser=mockParser
        )
    ...
    def test_parseLinkFlags(self):
        assert SFRRecordManager.parseLinkFlags(
    ...
    def test_agentParser_single_agent(self, testInstance):
        testAgents = testInstance.agentParser(
            ["Test|||author"], ["name", "viaf", "lcnaf", "role"]
        )

        assert testAgents == [
            {"name": "Test", "viaf": "", "lcnaf": "", "roles": ["author"]}
        ]

    def test_agentParser_multiple_agents(self, testInstance):
        inputAgents = ["Author|||author", "Publisher|1234||publisher"]
        testAgents = testInstance.agentParser(
            inputAgents, ["name", "viaf", "lcnaf", "role"]
        )
    ...
    def test_agentParser_multiple_agents_overlap(self, testInstance):
        inputAgents = [
            "Author|||author",
            "Publisher|1234||publisher",
            "Pub Alt Name|1234|n9876|other",
        ]
        testAgents = testInstance.agentParser(
            inputAgents, ["name", "viaf", "lcnaf", "role"]
        )
    ...
    def test_agentParser_multiple_agents_empty_and_overlap(self, testInstance):
        inputAgents = [
            "Author||n9876|author",
            "Author Alt|1234|n9876|illustrator",
            "|other",
        ]
        testAgents = testInstance.agentParser(
            inputAgents, ["name", "viaf", "lcnaf", "role"]
    ...
    def test_agentParser_multiple_agents_jw_match(self, testInstance):
        inputAgents = [
            "Author T. Tester|||author",
            "Author Tester (1950-)|||illustrator",
            "|other",
        ]
        testAgents = testInstance.agentParser(
            inputAgents, ["name", "viaf", "lcnaf", "role"]
    ...
    def test_normalizeDates(self, testInstance):
        testDates = testInstance.normalizeDates(
            ["1999.|test", "2000|other", "sometime 1900-12 [pub]|other"]
        )
