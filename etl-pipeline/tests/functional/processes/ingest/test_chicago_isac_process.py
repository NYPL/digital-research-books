...
def test_chicago_isac_process(db_manager, s3_manager, mock_epub_to_webpub, mock_sqs_manager):
    isac_process = ChicagoISACProcess('complete', None, None, None, 5, None)
    number_of_records_ingested = isac_process.runProcess()

    records = assert_ingested_records(db_manager, sources=[Source.CHICACO_ISAC.value], expected_number_of_records=number_of_records_ingested)
