...
def test_fulfill_links(
    db_manager, redis_manager, s3_manager, limited_access_record_uuid
):
    record_clustered = RecordClusterer(
        db_manager=db_manager, redis_manager=redis_manager
    )

    record = (
        db_manager.session.query(Record)
        .filter_by(uuid=limited_access_record_uuid)
        .first()
    )
    ...
    epub_link = next(
        (link for link in item.links if link.media_type == "application/epub+zip"), None
    )
    ...
    manifest_key = (
        f"manifests/publisher_backlist/test_{limited_access_record_uuid}.json"
    )
        ...
        str(
            Part(
                ...
                url=get_stored_file_url(
                    storage_name=os.environ["FILE_BUCKET"], file_path=manifest_key
                ),
                source=record.source,
                file_type="application/webpub+json",
                flags=str(
                    FileFlags(reader=True, nypl_login=True, fulfill_limited_access=True)
                ),
            )
        ),
    )
    ...
    manifest_file = s3_manager.client.get_object(
        Bucket=os.environ["FILE_BUCKET"], Key=manifest_key
    )
    ...
    s3_manager.client.delete_object(Bucket=os.environ["FILE_BUCKET"], Key=manifest_key)
