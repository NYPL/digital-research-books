...
def test_s3_bucket():
    s3 = boto3.client("s3")
    assert test_s3_bucket_read_access(s3, TEST_BUCKET), f"Bucket {TEST_BUCKET} is not readable"
...
def test_download_barcode(db_manager):
    ...
    assert service.bucket == TEST_BUCKET, (f"Expected bucket name '{TEST_BUCKET}', but got {service.bucket}")
