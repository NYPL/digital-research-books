...
class TestOCLCCatalogManager:
    ...
    def test_query_bibs(self, test_instance: OCLCCatalogManager):
        ...
        if test_instance.rate_limited:
            pytest.skip("OCLC rate limited (429)")
        if test_instance.service_unavailable:
            pytest.skip("OCLC service unavailable (5xx or network error)")
