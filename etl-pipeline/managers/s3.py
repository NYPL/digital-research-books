...
class S3Manager:
    ...
    def store_pdf_manifest(self, record: Record, bucket_name):
        ...
        pdf_part = next(
            filter(lambda part: part.file_type == "application/pdf", record.parts), None
        )
        manifest_part = next(
            filter(
                lambda part: part.file_type == "application/webpub+json", record.parts
            ),
            None,
        )
        ...
        if manifest_part is not None and pdf_part is not None:
            try:
                self.client.head_object(
                    Bucket=manifest_part.file_bucket, Key=manifest_part.file_key
                )
            except ClientError:
            ...
            manifest_json = self.generate_manifest(
                record=record, source_url=pdf_part.url, manifest_url=manifest_part.url
            )
            self.create_manifest_in_s3(
                manifest_path=manifest_part.file_key,
            ...
            manifest_url = get_stored_file_url(
                storage_name=bucket_name, file_path=manifest_path
            )
            manifest_json = self.generate_manifest(
                record=record, source_url=pdf_part.url, manifest_url=manifest_url
            )
    ...
    def generate_manifest(self, record: Record, source_url: str, manifest_url: str):
        ...
        manifest.links.append(
            {"rel": "self", "href": manifest_url, "type": "application/webpub+json"}
        )
    ...
    def put_object(
        ...
        if get_object_response and (
            get_object_response["ResponseMetadata"]["HTTPStatusCode"] == 304
            or get_object_response["Metadata"].get("md5checksum", None) == object_md5
        ):
    ...
    def store_epub(self, object, key: str, bucket: str):
        ...
        with ZipFile(BytesIO(object), "r") as epub_zip:
            for component in epub_zip.namelist():
                self.put_object(
                    object=epub_zip.open(component).read(),
                    key=f"{key_prefix}/{component}",
                    bucket=bucket,
                )
        ...
        try:
            if md5_hash:
                return self.client.get_object(
                    Bucket=bucket, Key=key, IfNoneMatch=md5_hash
                )
