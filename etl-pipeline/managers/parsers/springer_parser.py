...
class SpringerParser(ParserABC):
    ...
    def create_pdf_links(self, s3_root):
        pdf_source_uri = "https://link.springer.com/content/pdf/{}/{}.pdf".format(
            self.code, self.uri_identifier
        )

        if SpringerParser.check_availability(pdf_source_uri) is False:
            return []
    ...
    def create_epub_links(self, s3_root):
        epub_source_uri = "https://link.springer.com/download/epub/{}/{}.epub".format(
            self.code, self.uri_identifier
        )

        if SpringerParser.check_availability(epub_source_uri) is False:
            return []
