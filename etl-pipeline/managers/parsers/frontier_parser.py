...
class FrontierParser(ParserABC):
    ...
    def generate_epub_links(self, s3_root):
        epub_source_uri = 'https://www.frontiersin.org/research-topics/{}/epub'.format(self.uri_identifier)

        frontier_status, frontier_headers = FrontierParser.check_availability(epub_source_uri)

        if frontier_status != 200: return [None]
        ...
        epub_download_path = 'epubs/frontier/{}_{}.epub'.format(self.uri_identifier, filename)
        epub_download_uri = '{}{}'.format(s3_root, epub_download_path)

        epub_read_path = 'epubs/frontier/{}_{}/META-INF/container.xml'.format(self.uri_identifier, filename)
        epub_read_uri = '{}{}'.format(s3_root, epub_read_path)

        webpub_read_path = 'epubs/frontier/{}_{}/manifest.json'.format(self.uri_identifier, filename)
    ...
    def generate_pdf_links(self, s3_root):
        pdf_source_uri = 'https://www.frontiersin.org/research-topics/{}/pdf'.format(self.uri_identifier)
