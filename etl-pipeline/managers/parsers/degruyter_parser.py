...
class DeGruyterParser(ParserABC):
    ...
    def create_links(self):
        ...
        isbn_match = re.search(
            r"\/(document\/doi\/10\.[0-9]+\/([0-9]+))\/html", self.uri
        )

        if title_match:
            self.uri_identifier = title_match.group(1)
            epub_source_uri = "https://www.degruyter.com/downloadepub/title/{}".format(
                self.uri_identifier
            )
            pdf_source_uri = "https://www.degruyter.com/downloadpdf/title/{}".format(
                self.uri_identifier
            )
        elif isbn_match:
    ...
    def generate_epub_links(self, s3_root, epub_source_uri):
        ...
        epub_read_path = "epubs/degruyter/{}/META-INF/container.xml".format(
            self.uri_identifier
        )
        epub_read_uri = "{}{}".format(s3_root, epub_read_path)

        webpub_read_path = "epubs/degruyter/{}/manifest.json".format(
            self.uri_identifier
        )
    ...
    def make_head_query(uri):
        head_resp = requests.head(
            uri,
            timeout=DeGruyterParser.TIMEOUT,
