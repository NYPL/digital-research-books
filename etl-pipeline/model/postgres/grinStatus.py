...
class GRINStatus(Base, Core):
    ...
    failed_download = Column(INTEGER, default=0)  # Tracks how many times have we failed to download a specific file
    state = Column(
        Unicode,
        ENUM('pending_conversion','converting', 'converted', 'downloaded', 
             name='state', create_type=False),
        nullable=False
    )
