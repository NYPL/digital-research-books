...
class LOCService(SourceService):
    def get_records(
        ...
        yield from self._import_records(
            loc_collection_url=LOC_ROOT_DIGIT,
            start_timestamp=start_timestamp,
            limit=limit,
        )
    ...
    def _import_records(
        ...
        try:
            while (
                page_json_data := self._fetch_page_json_data(
                    page_url=f"{loc_collection_url}&sp={page_number}"
                )
            ) is not None:
                for record_data in page_json_data.get("results", []):
                    if start_timestamp:
                        record_timestamp = datetime.strptime(
                            record_data["timestamp"], "%Y-%m-%dT%H:%M:%S.%fZ"
                        )
