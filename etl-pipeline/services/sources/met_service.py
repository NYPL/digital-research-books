...
class METService(SourceService):
    ...
    def get_records(
        ...
        while met_records := self._get_met_records(
            page_size=page_size, current_position=current_position
        ):
            for record in met_records:
                ...
                if (
                    start_timestamp
                    and datetime.strptime(modified_at, "%Y-%m-%d") >= start_timestamp
                ) or "copyright" in rights.lower():
                    continue
    ...
    def _get_met_records(self, page_size: int = 50, current_position: int = 0) -> list:
        try:
            records_response = self.query_met_api(
                query=self.LIST_QUERY.format(page_size, current_position)
            )
