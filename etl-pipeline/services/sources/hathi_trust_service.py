...
class HathiTrustService(SourceService):
    ...
    FIELD_SIZE_LIMIT = 131072 * 128  # ~16MB to handle large HathiTrust metadata fields
    ...
    def get_records(
        ...
        for data_file in data_files:
            ...
            if (start_timestamp is None and not data_file.get("full")) or (
                start_timestamp
                and parser.parse(data_file.get("modified")).replace(tzinfo=None)
                < start_timestamp
            ):
                continue
    ...
    def _get_data_files(self) -> list[dict]:
        ...
        data_files.sort(
            key=lambda x: parser.parse(x["created"]).timestamp(), reverse=True
        )
    ...
    def _get_data_from_file_url(self, file_url: str):
        try:
            ...
            logger.exception(
                f"Received HTTP error with status code: {data_file_response.status_code} from {file_url}"
            )
        except Exception:
    ...
    def _is_ingestable(self, data_row, start_datetime: datetime | None) -> bool:
        ...
        return (
            rights
            and rights not in self.HATHI_RIGHTS_SKIPS
            and (not start_datetime or date_modified >= start_datetime)
        )
    ...
    def _get_date_modified(self, data_row) -> datetime:
        ...
        try:
            return (
                datetime.strptime(date_modified, "%Y-%m-%d %H:%M:%S")
                if date_modified
                else datetime.now()
            )
        except ValueError:
