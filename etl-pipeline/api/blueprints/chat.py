...
def prepare_search_response(search_results: SearchResults) -> Optional[SearchResponse]:
    ...
    Returns None if search_results is empty (no search was executed).
    `formatted_search_result` is consumable by flask.jsonify.
    `tool_call_id` is the call_id of the search tool call that produced the result.
    """
    ...
    if len(search_results) > 1:
        ...
        tool_call_id, search_result = list(search_results.items())[-1]
    elif len(search_results) == 1:
        tool_call_id, search_result = list(search_results.items())[0]
    else:
    ...
    if result_type is None:
        raise ValueError(f"Unsupported search tool type: {search_result['tool_name']}")
    ...
    for edition_result in search_result["edition_data"]:
        ...
        if result_type == "catalogSearch":
            ...
            edition_response.update(
                {"barcode": edition_result.barcode, **edition_metadata, **work_metadata}
            )
    ...
    # redundant metadata in the contentSearch editions or response objects.
        ...
        logger.info(f"Returning {len(snippets)} snippets in content search response")

    return SearchResponse(result_type, formatted_search_result, tool_call_id)
...
def chat(session_id):
    ...
    with LogContextVars(get_app_logger(), context=log_context):
        try:
            ...
            search_response = prepare_search_response(
