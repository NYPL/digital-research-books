...
def _get_identifiers(oclc_bib, oclc_number):
    ...
    return [
        f"{id}|{id_type}" 
        for id, id_type in [(oclc_number, 'oclc'), (owi_number, 'owi'), (edition_cluster, 'oec')] 
        if id is not None
    ]
...
def _get_parts(oclc_bib):
    ...
    return [
        str(Part(index=1, url=uri, source='oclc', file_type='text/html', flags=str(FileFlags(embed=True))))
