...
class OCLCBibMapping(BaseMapping):
    ...
    def _get_authors(self, creators):
        ...
        return list(
            filter(
                lambda creator: creator.get("isPrimary", False)
                or self._is_author(creator),
                creators,
            )
        )
    ...
    def _get_contributors(self, creators):
        ...
        return list(
            filter(
                ...
                and not self._is_author(creator),
                creators,
            )
        )
    ...
    def _is_author(self, creator):
        for role in set(
            map(
                lambda relator: relator.get("term", "").lower(),
                creator.get("relators", []),
            )
        ):
