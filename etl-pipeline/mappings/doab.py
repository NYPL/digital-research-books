...
class DOABMapping(BaseMapping):
    ...
    def map_doab_record(self, record, namespaces) -> Record:
        ...
        if header is not None:
            identifier = header.find(
                ".//identifier", namespaces=DSpaceService.ROOT_NAMESPACE
            )
        ...
        title = doab_record.xpath(
            "./dc:title/text()", namespaces=namespaces
        ) + doab_record.xpath("./datacite:creator/text()", namespaces=namespaces)
        ...
        return Record(
            ...
            spatial=doab_record.xpath(
                "./oapen:placepublication/text()", namespaces=namespaces
            ),
    ...
    def _get_text_type_data(self, record, namespaces, field_xpath, format_string):
        ...
        data_tuples = [
            (item.text, item.get("type")) for item in field_data if item.get("type")
        ]

        return [format_string.format(item[0], item[1]) for item in data_tuples]

    def _get_authors(self, record, namespaces):
        datacite_authors = record.xpath(
            "./datacite:creator/text()", namespaces=namespaces
        )
    ...
    def _get_identifers(self, record, namespaces):
        ...
        datacite_ids = record.xpath(
            "./datacite:identifier/text()", namespaces=namespaces
        )

        datacite_alt_ids = self._get_text_type_data(
            record, namespaces, "./datacite:alternateIdentifier", "{}|{}"
        )
        dc_alt_ids = self._get_text_type_data(
            record, namespaces, "./dc:alternateIdentifier", "{}|{}"
        )
    ...
    def _get_contributors(self, record, namespaces):
        return self._get_text_type_data(
            record, namespaces, "./datacite:contributor", "{}|||{}"
        )

    def _get_dates(self, record, namespaces):
        datacite_dates = self._get_text_type_data(
            record, namespaces, "./datacite:date", "{}|||{}"
        )
    ...
    def _get_subjects(self, record, namespaces):
        datacite_subjects = record.xpath(
            "./datacite:subject/text()", namespaces=namespaces
        )
    ...
    def _get_links(self):
        ...
        for manifest in self.link_manager.manifests:
            ...
            self.store_manager.create_manifest_in_s3(
            ...
            manifest_url = get_stored_file_url(
                storage_name=self.file_bucket, file_path=manifest_path
            )
            self.record.has_part.insert(
                0,
                str(
                    Part(
                        index=1,
        ...
        for epub_link in self.link_manager.epub_links:
            ...
            epub_url = get_stored_file_url(
                storage_name=self.file_bucket, file_path=epub_path
            )
    ...
    def _get_rights(self, record, namespaces):
        license_conditions = self._get_uri_text_data(
            record, namespaces, "./oaire:licenseCondition", "doab|{}||{}"
        )

        datacite_rights = self._get_uri_text_data(
            record, namespaces, "./datacite:rights", "doab|{}||{}"
        )

        dc_rights = self._get_uri_text_data(
            record, namespaces, "./dc:rights", "doab|{}||{}"
        )
