...
class GutenbergMapping(XMLMapping):
    ...
    def createMapping(self):
        return {
            ...
            "rights": (
                ["//dcterms:rights/text()", "*/cc:license/@rdf:resource"],
                "gutenberg|{1}||{0}|",
            ),
            "identifiers": [
            ...
            "is_part_of": [
                ("//pgterms:bookshelf/rdf:Description/rdf:value/text()", "{0}||series")
            ],
        }
    ...
    def applyFormatting(self):
        ...
        for i, subject in enumerate(self.record.subjects):
            ...
            authority = (
                subjComponents[0].replace("http://purl.org/dc/terms/", "").lower()
            )
            self.record.subjects[i] = "{}|{}|{}".format(authority, *subjComponents[1:])
        ...
        for i, author in enumerate(self.record.authors):
            ...
            self.record.authors[i] = "|".join(
                [name.replace(" (-)", "")] + authorComponents
            )
