...
def map_loc_record(source_record: dict) -> Optional[Record]:
    ...
    return Record(
        ...
        authors=[f"{author}|||true" for author in source_record.get("contributor", [])]
        if source_record.get("contributor")
        else None,
        dates=[f"{source_record.get('date')}|publication_date"]
        if source_record.get("date")
        else None,
        ...
        identifiers=[
            ...
            for id in [
                f"{lccn}|lccn",
                f"{item_details.get('call_number')[0]}|call_number"
                if item_details.get("call_number")
                else None,
            ]
            if id is not None
        ],
        contributors=[
            ...
            for contributor in source_record.get("contributor", [])
        ]
        if source_record.get("contributor")
        else None,
        extent=item_details.get("medium")[0] if item_details.get("medium") else None,
        subjects=[f"{subject}||" for subject in item_details.get("subjects")]
        if item_details.get("subjects")
        else None,
        languages=[f"||{language}" for language in item_details.get("language")]
        if item_details.get("language")
        else None,
        rights=get_rights_string(
            rights_source="loc", license=item_details.get("rights_advisory")[0]
        )
        if item_details.get("rights_advisory")
        else None,
        abstract=source_record.get("description")[0]
        if source_record.get("description")
        else None,
        is_part_of=[f"{part_of}|collection" for part_of in source_record.get("partof")]
        if source_record.get("partof")
        else None,
        spatial=item_details.get("location")[0]
        if item_details.get("location")
        else None,
...
def _parse_publishers(
    ...
    for created_published_data in created_published_list:
        if ":" not in created_published_data:
            ...
            if (
                len(publisher_and_location_data) >= 2
                and "," in publisher_and_location_data[1]
            ):
...
def _get_has_part(first_file_resource: dict, source_id: str) -> list[str]:
    ...
    if "epub_file" in first_file_resource.keys():
        ...
        epub_url = get_stored_file_url(
            storage_name=os.environ["FILE_BUCKET"], file_path=epub_location
        )
