...
const ResearchAssistantViewer: React.FC<{ pdfData: ApiItemsRead }> = ({
  pdfData,
}) => {
  ...
  useEffect(() => {
    const newPdfUrl = URL.createObjectURL(base64ToBlob(currentPdfData.pageData, 'application/pdf')) + '#toolbar=0&navpanes=0&scrollbar=0&view=fitH';
    setPdfUrl(newPdfUrl);
  }, [currentPdfData])
  ...
  const fetchPage = async (pageUrl) => {
    setIsLoading(true);
    try {
      const nextItemReadResults = await itemsReadFetcher(pageUrl);
